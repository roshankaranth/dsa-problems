...
void insert(TrieNode *root, const char *key){
    ...
    for (level = 0; level < length; level++){
        char ch = key[level];
        bool found = false;
        for(int i = 0 ; i < pCrawl->childCount; i++){
            if(pCrawl->character[i] == ch){
                found = true;
                index = i;
                break;
            }
        ...
        if(!found){
            pCrawl->character = (char*)realloc(pCrawl->character,sizeof(char)*(pCrawl->childCount+1));
            pCrawl->children = (TrieNode**)realloc(pCrawl->children, sizeof(TrieNode*)*(pCrawl->childCount + 1));
            pCrawl->childCount++;

            pCrawl->character[pCrawl->childCount-1] = ch;
            pCrawl->children[pCrawl->childCount-1] = getNode();
            index = pCrawl->childCount-1;
        }

        if(found && pCrawl->children[index] == NULL){
